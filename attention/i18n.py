from __future__ import annotations

from typing import Any

from .constants import DEFAULT_LANGUAGE

LANG_STRINGS: dict[str, dict[str, str]] = {
    "en": {
        "app_title": "Task Reminder",
        "tray_show": "Show Task",
        "tray_hide": "Hide Window",
        "tray_edit": "Edit Task...",
        "tray_quit": "Quit",
        "tray_history": "View History",
        "settings_title": "Task Settings",
        "button_save": "Save",
        "button_cancel": "Cancel",
        "error_empty": "Task text cannot be empty.",
        "error_save": "Failed to save configuration:\\n{error}",
        "error_invalid_color": "Invalid color format. Use #RRGGBB.",
        "error_invalid_transparency": "Invalid transparency value. Use 0.2 - 1.0.",
        "error_history_save": "Failed to write history:\\n{error}",
        "error_lock_failed": "Failed to lock the workstation. Please check system permissions.",
        "error_autostart": "Failed to update startup setting:\\n{error}",
        "notice_title": "Notice",
        "label_text": "Task Text",
        "label_font": "Font Family",
        "label_font_size": "Font Size",
        "label_text_color": "Text Color (#RRGGBB)",
        "label_outline_color": "Outline Color (#RRGGBB)",
        "label_transparency": "Transparency (0.2 - 1.0)",
        "label_language": "Language",
        "tray_autostart": "Start on Boot",
        "prompt_estimate_title": "Estimated Time",
        "prompt_estimate_message": "Enter minutes (optional):",
        "estimate_label": "Estimated {minutes} min",
        "estimate_over_label": "Exceeded {minutes} min",
        "label_schedule_button": "Manage Schedule...",
        "language_option_en": "English",
        "language_option_zh": "Chinese",
        "menu_start": "Start Task",
        "menu_new": "New Task...",
        "menu_pause": "Pause Task",
        "menu_resume": "Resume Task",
        "menu_stop": "Stop Task",
        "menu_edit": "Edit Task...",
        "menu_history": "Open Task History",
        "menu_tasks": "Task List...",
        "prompt_start_title": "Start Task",
        "prompt_start_message": "Enter task name:",
        "prompt_edit_title": "Edit Task",
        "prompt_edit_message": "Update task text:",
        "pause_prefix": "Paused",
        "no_task": "No task",
        "todo_title": "Task List",
        "todo_switch": "Switch",
        "todo_add": "New",
        "todo_edit": "Edit",
        "todo_stop": "Terminate",
        "todo_empty": "No saved tasks.",
        "todo_current": "Current",
        "todo_status_running": "Running",
        "todo_status_paused": "Paused",
        "todo_status_saved": "Saved",
        "history_title": "Task History",
        "history_close": "Close",
        "history_empty": "No records for today.",
        "history_column_time": "Time",
        "history_column_event": "Event",
        "history_column_title": "Task",
        "history_date_label": "Date",
        "history_event_start": "Started",
        "history_event_pause": "Paused",
        "history_event_resume": "Resumed",
        "history_event_stop": "Stopped",
        "time_started": "Start Time",
        "time_elapsed_less_minute": "Elapsed <1 minute",
        "time_elapsed_minutes": "Elapsed {minutes} minutes",
        "time_elapsed_hours": "Elapsed {hours}h {minutes}m",
        "time_elapsed_hours_only": "Elapsed {hours} hours",
        "schedule_title": "Schedule",
        "schedule_add": "Add",
        "schedule_edit": "Edit",
        "schedule_delete": "Delete",
        "schedule_label": "Label",
        "schedule_start": "Start (HH:MM)",
        "schedule_end": "End (HH:MM)",
        "schedule_no_selection": "Select an entry first.",
        "schedule_default_label": "Break",
        "confirm_delete_schedule": "Delete the selected schedule entry?",
        "error_invalid_time": "Invalid time format. Use HH:MM.",
        "error_time_order": "End time must be later than start time.",
        "overlay_current": "Current: {label} ({start} - {end})",
        "overlay_remaining": "Remaining {time}",
        "overlay_schedule_title": "Today's Schedule",
        "overlay_next": "Next: {label} ({start} - {end})",
        "schedule_prelock_notice": "Upcoming lock for {label} at {start}-{end} in 30 minutes.",
    },
    "zh": {
        "app_title": "任务提醒",
        "tray_show": "显示任务",
        "tray_hide": "隐藏窗口",
        "tray_edit": "编辑任务...",
        "tray_quit": "退出",
        "tray_history": "查看历史",
        "settings_title": "任务设置",
        "button_save": "保存",
        "button_cancel": "取消",
        "error_empty": "任务内容不能为空。",
        "error_save": "保存配置失败：\n{error}",
        "error_invalid_color": "颜色格式不正确，请使用 #RRGGBB。",
        "error_invalid_transparency": "透明度数值不正确，范围 0.2 - 1.0。",
        "error_history_save": "写入历史失败：\n{error}",
        "error_lock_failed": "自动锁屏失败，请检查系统权限。",
        "error_autostart": "设置开机自启失败：\n{error}",
        "notice_title": "提示",
        "label_text": "任务内容",
        "label_font": "字体",
        "label_font_size": "字号",
        "label_text_color": "文字颜色 (#RRGGBB)",
        "label_outline_color": "描边颜色 (#RRGGBB)",
        "label_transparency": "透明度 (0.2 - 1.0)",
        "label_language": "语言",
        "tray_autostart": "开机自启",
        "prompt_estimate_title": "预估时长",
        "prompt_estimate_message": "请输入预估分钟数（可选）",
        "estimate_label": "预估 {minutes} 分钟",
        "estimate_over_label": "已超出 {minutes} 分钟",
        "label_schedule_button": "管理时间表...",
        "language_option_en": "英语",
        "language_option_zh": "中文",
        "menu_start": "开始任务",
        "menu_new": "新建任务...",
        "menu_pause": "暂停任务",
        "menu_resume": "继续任务",
        "menu_stop": "终止任务",
        "menu_edit": "编辑任务...",
        "menu_history": "查看任务历史",
        "menu_tasks": "待办清单...",
        "prompt_start_title": "开始任务",
        "prompt_start_message": "请输入任务名称：",
        "prompt_edit_title": "编辑任务",
        "prompt_edit_message": "修改任务内容：",
        "pause_prefix": "暂停中",
        "no_task": "暂无任务",
        "todo_title": "待办清单",
        "todo_switch": "切换",
        "todo_add": "新建",
        "todo_edit": "编辑",
        "todo_stop": "终止",
        "todo_empty": "暂无待办任务。",
        "todo_current": "当前",
        "todo_status_running": "进行中",
        "todo_status_paused": "已暂停",
        "todo_status_saved": "已保存",
        "history_title": "任务历史",
        "history_close": "关闭",
        "history_empty": "今天还没有任务记录。",
        "history_column_time": "时间",
        "history_column_event": "状态",
        "history_column_title": "任务",
        "history_date_label": "日期",
        "history_event_start": "已开始",
        "history_event_pause": "已暂停",
        "history_event_resume": "已继续",
        "history_event_stop": "已终止",
        "time_started": "开始时间",
        "time_elapsed_less_minute": "已过去不足1分钟",
        "time_elapsed_minutes": "已过去{minutes}分钟",
        "time_elapsed_hours": "已过去{hours}小时{minutes}分",
        "time_elapsed_hours_only": "已过去{hours}小时",
        "schedule_title": "时间表",
        "schedule_add": "新增",
        "schedule_edit": "编辑",
        "schedule_delete": "删除",
        "schedule_label": "标签",
        "schedule_start": "开始时间 (HH:MM)",
        "schedule_end": "结束时间 (HH:MM)",
        "schedule_no_selection": "请先选择一条记录。",
        "schedule_default_label": "休息",
        "overlay_current": "当前：{label}（{start} - {end}）",
        "overlay_remaining": "剩余 {time}",
        "overlay_schedule_title": "今日时间表",
        "overlay_next": "下一个：{label}（{start} - {end}）",
        "confirm_delete_schedule": "确定删除选中的时间段吗？",
        "error_invalid_time": "时间格式不正确，请使用 HH:MM。",
        "error_time_order": "结束时间必须晚于开始时间。",
        "schedule_prelock_notice": "将在30分钟后锁屏：{label}（{start}-{end}），请提前保存。",
    },
}

SUPPORTED_LANGUAGES = tuple(LANG_STRINGS.keys())
NO_TASK_VALUES = {data["no_task"] for data in LANG_STRINGS.values()}


def get_strings(language: str) -> dict[str, str]:
    return LANG_STRINGS.get(language, LANG_STRINGS[DEFAULT_LANGUAGE])


def translate(language: str, key: str, **kwargs: Any) -> str:
    strings = get_strings(language)
    template = strings.get(key, key)
    return template.format(**kwargs)


def strip_pause_prefix(text: str) -> str:
    for data in LANG_STRINGS.values():
        prefix = data["pause_prefix"]
        if text.startswith(prefix):
            return text[len(prefix) :].lstrip()
    return text

